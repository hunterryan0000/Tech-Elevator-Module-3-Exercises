...
public class JdbcPostDao implements PostDao {
    ...
    public Post createPost(Post postData) {
        String sql = "INSERT INTO post (title, text, user_id) VALUES (?, ?, ?) RETURNING id;";
        Integer id = jdbcTemplate.queryForObject(sql, Integer.class, postData.getTitle(), postData.getText(), postData.getUserId());
        return getPost(id);
    }
    ...
    public List<Post> getPostsForUser(int userId) {
        String sql = "SELECT * FROM post " +
                "WHERE user_id = ? OR user_id IN ( " +
                    "(SELECT user2_id as friend_id FROM friend WHERE user1_id = ?) " +
                    "UNION (SELECT user1_id as friend_id FROM friend WHERE user2_id = ?) " +
                ") ORDER BY create_date;";
        SqlRowSet resultSet = jdbcTemplate.queryForRowSet(sql, userId, userId, userId);
