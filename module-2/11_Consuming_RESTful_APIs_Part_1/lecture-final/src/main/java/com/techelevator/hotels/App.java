    ...
    private void run() {
        ...
        while (menuSelection != 0) {
            ...
            if (menuSelection == 1) {
                ...
                // Get the list of hotels from the HotelService
                Hotel[] hotels = hotelService.listHotels();

                // Use the provided ConsoleService to display the information
                consoleService.printHotels(hotels);

            } else if (menuSelection == 2) {

                // Get the reviews and then display them
                Review[] reviews = hotelService.listReviews();
                consoleService.printReviews(reviews);

            } else if (menuSelection == 3) {

                // Get hotel 1 and display
