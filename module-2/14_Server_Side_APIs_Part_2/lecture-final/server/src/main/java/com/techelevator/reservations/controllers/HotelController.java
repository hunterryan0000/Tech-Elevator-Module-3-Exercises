...
public class HotelController {
    ...
    // Dependency injection (doesn't have to be through a constructor)
    @Autowired
    ...
    // Dependency injection - the controller DEPENDS ON these DAOs to access the data
    //  - Take in the dependencies through our constructor
    //  - Programming to our interface (pass in the Interface type - not the specific concrete class)

    // - when using @Autowired on the 2 DAO properties, we don't need the constructor to do the dependency injection
...
//        this.reservationDao = new MemoryReservationDao(hotelDao);
    ...
    public Reservation updateReservation(@Valid @RequestBody Reservation reservation, @PathVariable int id) throws ReservationNotFoundException {
        Reservation updated = reservationDao.update(reservation, id);
        return updated;
    }
