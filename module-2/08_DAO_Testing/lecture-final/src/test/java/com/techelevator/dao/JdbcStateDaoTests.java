...
public class JdbcStateDaoTests extends BaseDaoTests {
    ...
    public void getStates_returns_all_states() {
        ...
        // Checking we got the correct size of results
        // we can look at the test data insert script to see there are 3
        Assert.assertEquals(3, states.size());

        // We know that the query should order by state abbreviation, so we can validate the order is correct
