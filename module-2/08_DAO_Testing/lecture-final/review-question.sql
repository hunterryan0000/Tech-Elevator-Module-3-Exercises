...
CREATE TABLE park (
	park_id serial PRIMARY KEY, -- Create PK with declaration, gives a default name
...
-- CONSTRAINT fk_yada_ya foreign key x references y(z) 
...
CREATE TABLE park_state (
	...
	FOREIGN KEY(state_abbreviation) REFERENCES state(state_abbreviation) -- Can leave off `constraint` w/ name to get a default
);
