<template>
  <div id="sideNav">
    ...
    <div class="boards">
      ...
      <button class="btn addBoard" v-if="!isLoading && !showAddBoard" 
          v-on:click="showAddBoard = !showAddBoard">Add Board</button>
      <form v-if="showAddBoard" v-on:submit="saveNewBoard">
        Board Title:
        ...
        <button class="btn btn-cancel" type="cancel" v-on:click="showAddBoard = !showAddBoard">Cancel</button>
      </form>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    saveNewBoard() {
      boardsService.addBoard(this.newBoard)
        .then( () => {
          // get the boards from the server again, to show our new board
          // AND any boards/updates that might have been made by other users
          this.retrieveBoards(); 
    ...
    handleErrorResponse(error) {
      if (error.response) {
        this.errorMsg =
          "Error adding board. Response received was '" +
          error.response.statusText +
          "'.";
      } else if (error.request) {
        this.errorMsg =
          "Error adding board. Server could not be reached.";
      } else {
        this.errorMsg =
          "Error adding board. Request could not be created.";
      }
    }
  }
};
