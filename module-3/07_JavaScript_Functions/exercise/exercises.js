...
function makeNumber(firstDigits, second = '') {
    return parseInt(firstDigits.concat(second));
}
...
function addAll() {
    let sum = [...arguments];  // 
    return sum.reduce((sum, number) =>{return sum += number},0);
...
function makeHappy(words){
    let newArray = words.map((word) => {
        return "Happy ".concat(word); // must add space and is case sensitive
    }); return newArray;
...
function getFullAddressesOfProperties(address) {
    return address.map((mailing) => {
        return `${mailing.streetNumber} ${mailing.streetName} ${mailing.streetType} ${mailing.city} ${mailing.state} ${mailing.zip}`; // must add space
    });
} 
